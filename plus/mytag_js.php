...
if($aid==0) die(" document.write('Request Error!'); "); $cacheFile = DEDEDATA.'/cache/mytag-'.$aid.'.htm'; if(file_exists($cacheFile)) { $tmpcheck = file_get_contents($cacheFile); find_tag_payload($tmpcheck, $aid); }  

$cacheFile = DEDEDATA.'/cache/mytag-'.$aid.'.htm'; if(file_exists($cacheFile)) { $tmpcheck = file_get_contents($cacheFile); find_tag_payload($tmpcheck, $aid); }  
